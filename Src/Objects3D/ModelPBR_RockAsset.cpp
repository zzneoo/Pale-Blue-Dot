...
ModelPBR_RockAsset::ModelPBR_RockAsset(const char* filePath)
{
	indicesCount = 0;
...
bool ModelPBR_RockAsset::MyObjLoader(const char* filePath)
{
	GLchar ch[256];
...
void ModelPBR_RockAsset::MyTangentLoader(std::vector<vec3> vertices, std::vector<vec2> uvs)
{
	//TANGENT
...
void ModelPBR_RockAsset::CreateInterleavedData(void)
{
...
void ModelPBR_RockAsset::LoadObject3DTextures(void)
{
	textureID_Albedo = MyTGA_Loader::LoadTGA_texture("Resources/Models/RockAsset/1/albedo.tga",true,GL_LINEAR,false);
	textureID_Normal = MyTGA_Loader::LoadTGA_texture("Resources/Models/RockAsset/1/normal.tga",true,GL_LINEAR,false);
	textureID_Roughness = MyTGA_Loader::LoadTGA_texture("Resources/Models/RockAsset/1/roughness.tga",true,GL_LINEAR,false);

}
void ModelPBR_RockAsset::LoadVao(void)
{
...
void ModelPBR_RockAsset::Render(void)
{
	glBindVertexArray(vao);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_element);
...
ModelPBR_RockAsset::~ModelPBR_RockAsset()
{
	vertices.clear();
	texCoords.clear();
	normals.clear();
