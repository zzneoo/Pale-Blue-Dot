...
ModelTextured::ModelTextured(const char* filePath)
{
	indicesCount = 0;
...
bool ModelTextured::MyObjLoader(const char* filePath)
{
	GLchar ch[256];
...
void ModelTextured::MyTangentLoader(std::vector<vec3> vertices, std::vector<vec2> uvs)
{
	//TANGENT
...
void ModelTextured::CreateInterleavedData(void)
{
...
void ModelTextured::LoadObject3DTextures(void)
{
	textureID_Albedo = MyTGA_Loader::LoadTGA_texture("Resources/Textures/Cave/Albedo.tga", true, GL_LINEAR, false);
}
void ModelTextured::LoadVao(void)
{
...
void ModelTextured::Render(void)
{
	glBindVertexArray(vao);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_element);
