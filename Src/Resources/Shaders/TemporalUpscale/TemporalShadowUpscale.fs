...
void main(void)
{
	ivec2 tempID=ivec2(gl_FragCoord)%16;
	int id=tempID.y*16+tempID.x;

	vec2 velocity=texture(tSamplerCurrVelocity,TexCoord).xy;
	vec2 prevUV=TexCoord-velocity;

	ivec2 pixelOffset;
	pixelOffset.x = iBlockPixelID % 16;
	pixelOffset.y = iBlockPixelID / 16;

	vec4 finalColor;
	vec4 CurrColor=texture(tSamplerCurrFrameNearest,TexCoord);
	vec4 PrevColor=(abs(prevUV.x-0.5)>0.5)||(abs(prevUV.y-0.5)>0.5)?CurrColor:texture(tSamplerPrevFrame,prevUV);
