...
uniform sampler2D Sampler2D_Subsurface;
...
void main(void)
{
//	FragColorBW=vec4(1.0,0.0,0.0,1.0);//COLOR_ATTACHMENT1
	FragColorMask=vec4(1.0,0.0,0.0,1.0);
	//PBR data
	vec4 albedo = pow(texture(Sampler2D_Albedo,outTexCoord).rgba,vec4(1));
	if(albedo.a<0.1)
	discard;

	vec4 normalRough = texture(Sampler2D_Normal,outTexCoord);
	...
	vec3 radiance = vec3(2);
		...
		directLighting += (diffuseBRDF + specularBRDF) * radiance * NdotL;
				...
				//Ambient
		directLighting+=(0.3*pow(AO,1.0)*albedo.rgb);

//	FragColor =vec4(tonemapFilmic(directLighting),1.0);
	FragColor =vec4(directLighting*v3LightColor,smoothstep(0.0,0.2,albedo.a));

//	FragColor =vec4((vec3(1.0)),1.0);
//HeightFog
float SunTheta = dot(normalize(v3SunPos),vec3(0.0,1.0,0.0));
//FragColor.rgb =mix(mix(FragColor.rgb,vec3(0.8)*smoothstep(0.1,0.2,SunTheta),min(v3EyePos.y*0.000475,1.0)),FragColor.rgb,smoothstep(0.12,0.61,treeWorldHeight*0.0022));
FragColor.rgb =mix(FragColor.rgb,vec3(0.8)*smoothstep(0.1,0.2,SunTheta),min(v3EyePos.y*0.000475,1.0));
