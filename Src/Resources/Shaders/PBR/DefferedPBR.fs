...
void main(void)
{
	...
	vec3 WorldSpaceSurfaceTangent = normalize((m4ModelX*vec4(inTangent,0.0)).xyz);
	vec3 WorldSpaceSurfaceNormal = normalize((m4ModelX*vec4(inNormal,0.0)).xyz);
	...
	Out_AlbedoAO = vec4(Albedo.rgb*0.9,1.0);
	Out_SpecRoughAlphaDither = vec4(0.0,Roughness,1.0,0.0);
