...
uniform sampler2D tSampler_Albedo;
uniform sampler2D tSampler_LODBlend;
...
void main(void)
{
	...
	vec4 Albedo = texture(tSampler_Albedo,UV);
	float Alpha = texture(tSampler_LODBlend,UV).r;
		
	Out_WorldSpacePosition = vec4(worldPos.xyz,Alpha);
	Out_WorldSpaceNormal = vec4(Normal,Alpha);
	Out_AlbedoAO = vec4(Albedo.rgb,Alpha);
	Out_SpecRoughAlphaDither = vec4(0.0,finalRoughness,1.0,1.0);
	Out_Velocity = vec4(Velocity,0.0,Alpha);
