...
void main(void)
{
	...
	//vec3 Layer0 = mix(DefferedColor.rgb,Clouds.rgb,Clouds.a);//sky-FarClouds
	vec3 Layer0 = mix(BackgroundColor.rgb,Clouds.rgb,Clouds.a*CloudsHorizonMask);//sky-FarClouds
	vec3 Layer1 = mix(Layer0,DefferedColor.rgb,DefferedColor.a);//sky-FarClouds
	//vec3 Layer1 = Layer0*(1.0-pow(FlareColor.r,2.0))+FlareColor.rgb*clamp(FlareColor.r+1.0*FlareColor.r,0.0,1.0);//Flare

	//FragColor = vec4(mix(DefferedColor.rgb,FlareColor.rgb,FlareColor.r),1.0);
	vec4 FragColorGroundFromSpace = vec4(Layer1+BackgroundColor.rgb*(1.0-DefferedColor.a),1.0);
	vec4 FragColorGround = vec4(Layer1,1.0);
	vec4 FragColorSpace = vec4(mix(BackgroundColor.rgb,Clouds.rgb,Clouds.a),0.0);//PlaneClouds

  //  FragColor = mix(FragColorGround,FragColorSpace,fMixSpaceGround);
  //  FragColor.rgb = BackgroundColor.rgb;//bloodRiver
  // FragColor.rgb = BackgroundColor.rgb*(1.0-DefferedColor.a)+DefferedColor.rgb;//bloodRiver
  //  FragColor.rgb = mix(BackgroundColor.rgb,DefferedColor.rgb,DefferedColor.a);//final
    FragColor = mix(FragColorGround,FragColorSpace,fMixSpaceGround);
