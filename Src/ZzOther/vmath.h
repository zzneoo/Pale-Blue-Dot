...
static inline mat4 Perspective_FarZeroNearOne(float fovy, float aspect, float n, float f)
{
    float q = 1.0f / (float)tan(radians(0.5f * fovy));
    float A = q / aspect;
    float B = (-n ) / (f - n);
...
static inline mat4 Perspective_NearZeroFarOneLH(float fovy, float aspect, float n, float f)
{
    float q = 1.0f / (float)tan(radians(0.5f * fovy));
    float A = q / aspect;
    float B = (f ) / (f - n);
...
static inline mat4 Perspective_NearZeroFarOneTEST(float fovy, float aspect, float n, float f)
{
    float q = 1.0f / (float)tan(radians(0.5f * fovy));
    float A = q / aspect;
    float B = (f) / (f - n);
...
static inline mat4 PerspectiveInfinite_GLRH(float fovy, float aspect, float n)
{
    float q = 1.0f / (float)tan(radians(0.5f * fovy));
    float A = q / aspect;
    float B = -1.0f;
...
static inline mat4 PerspectiveInfinite_DXLH(float fovy, float aspect, float n)
{
    float q = 1.0f / (float)tan(radians(0.5f * fovy));
    float A = q / aspect;
    float B = 0.0f;
...
static inline mat4 inverse(mat4 mat)
{
...
static inline mat4 myInfiniteProj(float fovy, float aspect, float n, float f)
{
	...
	//code
	q = 1.0f / (float)tan(vmath::radians(0.5f * fovy));
	A = q / aspect;
	//B = (n + f) / (n - f);
	C = -2.0f *n;


	InfiniteMatrix[0] = vec4(   A, 0.0f,  0.0f,  0.0f);
	InfiniteMatrix[1] = vec4(0.0f,    q,  0.0f,  0.0f);
	InfiniteMatrix[2] = vec4(0.0f, 0.0f, -1.0f, -1.0f);
	InfiniteMatrix[3] = vec4(0.0f, 0.0f,     C,  0.0f);
